...
declare -a number_of_process=( 200 )
...
do
	...
	do
		...
		printf "####### ORGANIZATION OFF # comp/comm=%.1f # alpha=%.1f #######\n" $comp_per_comm $alpha
		counter=1
		for i in "${number_of_process[@]}"
		do
			./build/hpvc ./cluster2.xml ./deploymentOff.xml $number_of_vm $i $comp_per_comm $alpha > ./build/off.txt 2>/dev/null;
			...
			if [ $is_ok = 1 ]; then
				...
				echo $number_of_vm	$i	$slow_down	$simulation_time >> ./result/organizer_off_${comp_per_comm}_${alpha}.txt; # to save in file
			else
